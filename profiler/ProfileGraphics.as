	...
	public class ProfileGraphics extends Sprite
	{
		...
		public function updateGraphics( maxGraphWidth: int = -1 ): void
		{
			...
			// draw text
			{
				...
				out += sprintf( "%-" + ProfilerConfig.TreeColumnWidthChr + "s", tmp );

				// calls
				tmp = sprintf( "%d", stats.calls );
				out += sprintf( "%-8s", tmp );

				// time average (ms)
				if( stats.calls ) tmp = sprintf( "%3.2fms", Number( stats.totalTime ) / Number( stats.calls ) );
				else tmp = "0.00ms";
				out += sprintf( "%-12s", tmp );

				// percentage
				tmp = sprintf( "%3.2f%%", stats.percentage );
				out += sprintf( "%-12s", tmp );
